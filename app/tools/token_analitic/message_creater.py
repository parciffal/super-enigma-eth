...
class MessageCreater:
    ...
    CHAINS = {"ethereum": "1", "eth": "1"}
    CHAIN_FULL_NAMES = {"ethereum": "Ethereum", "eth": "Ethereum"}
    ...
    async def get_top_holders(self, data):
        ...
        if links.get("scan:link") != "":
            url = links.get("scan:link")
        else:
            url = None
        if data.get("goplus"):
            if data["goplus"].get("holders"):
                holders = data["goplus"]["holders"]
                msg = "⚖️Top holders: "
    ...
    async def get_liquidity(self, data):
        try:
            ...
            try:
                liquidity = await add_commas_to_float(
                    round(float(data["dexscreener"]["liquidity"]["usd"]), 2)
                )
                return f"🌊 LP:<b> ${liquidity}</b>\n"
            except (KeyError, ValueError):
                return ""
    ...
    async def check_quick_message(self, data, liquidity) -> str:
        count = 0
        try:
            if data["quick"]["is_Honeypot"] == False:
                count += 1
        except (KeyError, ValueError):
            pass
        try:
            if data["quick"]["is_Mintable"] == False:
                count += 1
        except (KeyError, ValueError):
            pass
        try:
            if data["quick"]["is_Proxy"] == False:
                count += 1
        except (KeyError, ValueError):
            pass
        try:
            if data["quick"]["can_Blacklist"] == False:
                count += 1
        except (KeyError, ValueError):
            pass
        try:
            if data["quick"]["contract_Verified"]:
                count += 1
        except (KeyError, ValueError):
            pass
    ...
    async def get_quick_message(self, data, liquidity):
        try:
            ...
            honeypot = (
                self.QUICK_BOOL[data["quick"]["is_Honeypot"]]
                if data["quick"].get("is_Honeypot") is not None
                else "<b>N/A</b>"
            )
            mintable = (
                self.QUICK_BOOL[data["quick"]["is_Mintable"]]
                if data["quick"].get("is_Mintable") is not None
                else "<b>N/A</b>"
            )
            proxy = (
                self.QUICK_BOOL[data["quick"]["is_Proxy"]]
                if data["quick"].get("is_Proxy") is not None
                else "<b>N/A</b>"
            )
            blacklisted = (
                self.QUICK_BOOL[data["quick"]["can_Blacklist"]]
                if data["quick"].get("can_Blacklist") is not None
                else "<b>N/A</b>"
            )

            in_dex = (
                self.QUICK_BOOL[False] if liquidity != "" else self.QUICK_BOOL[True]
            )
            contract_verified = (
                self.QUICK_REVERSE[data["quick"]["contract_Verified"]]
                if data["quick"].get("contract_Verified") is not None
                else f"{self.QUICK_REVERSE[False]}"
            )
    ...
    async def get_message_analytic(self, data, liquidity):
        try:
            if (
                data.get("goplus")
                and data.get("goplus").get("is_honeypot")
                or data.get("goplus").get("honeypot_with_same_creator")
            ):
                count = await self.check_get_message_analytic(data)
                test_msg = f"Contract Verified: {count}\n"
                try:
                    external_call = (
                        self.MSG_BOOL[data["goplus"]["external_call"]]
                        if data["goplus"].get("external_call")
                        else f"<b>N/A </b>"
                    )
                ...
                try:
                    is_contract_renounced = (
                        self.QUICK_REVERSE[
                            data["dextool"]["audit"]["is_contract_renounced"]
                        ]
                        if data["dextool"].get("audit")
                        else f"<b>N/A </b>"
                    )
                ...
                try:
                    unlimitedFees = (
                        self.QUICK_REVERSE[data["dextool"]["audit"]["unlimitedFees"]]
                        if data["dextool"].get("audit")
                        else f"<b>N/A </b>"
                    )
                ...
                try:
                    hidden_owner = (
                        self.MSG_BOOL[data["goplus"]["hidden_owner"]]
                        if data["goplus"].get("hidden_owner")
                        else f"<b>N/A </b>"
                    )
                ...
                try:
                    open_source = (
                        self.CHECK_BOOL[data["goplus"]["is_open_source"]]
                        if data["goplus"].get("is_open_source")
                        else f"{self.MSG_BOOL['1']}"
                    )
                ...
                try:
                    mintable = (
                        self.MSG_BOOL[data["goplus"]["is_mintable"]]
                        if data["goplus"].get("is_mintable")
                        else f"<b>N/A </b>"
                    )
                ...
                try:
                    is_whitelisted = (
                        self.CHECK_BOOL[data["goplus"]["is_whitelisted"]]
                        if data["goplus"].get("is_whitelisted")
                        else f"<b>N/A </b>"
                    )
                ...
                try:
                    blacklisted = (
                        self.MSG_BOOL[data["goplus"]["is_blacklisted"]]
                        if data["goplus"].get("is_blacklisted")
                        else f"<b>N/A </b>"
                    )
                ...
                try:
                    proxy = (
                        self.MSG_BOOL[data["goplus"]["is_proxy"]]
                        if data["goplus"].get("is_proxy")
                        else f"<b>N/A </b>"
                    )
                ...
                try:
                    in_dex = (
                        self.CHECK_BOOL[data["goplus"]["is_in_dex"]]
                        if data["goplus"].get("is_in_dex")
                        else f"{self.MSG_BOOL['1']}"
                    )
                ...
                try:
                    honeypot = (
                        self.MSG_BOOL[data["goplus"]["is_honeypot"]]
                        if data["goplus"].get("is_honeypot")
                        else f"<b>N/A </b>"
                    )
    ...
    async def get_base_liquidity(self, data):
        try:
            add_commas = await add_commas_to_float(
                data["dexsceener"]["liquidity"]["base"]
            )
            return f"🌊 LP:<b> ${add_commas}</b>\n"
        except (KeyError, ValueError):
            return ""

    async def get_price_change(self, data):
        try:
            num = '{:,}'.format(data['dexscreener']['volume']['h24'])
            return f"⏳ 24h-Vol:<b> ${num}</b>\n"
        except (KeyError, ValueError):
            return ""
        ...
        if data.get("goplus"):
            if data["goplus"].get("holders"):
                for holder in data["goplus"]["holders"]:
                    if holder.get("address", "").lower() in [
                        "0x000000000000000000000000000000000000dead",
                        "0x0000000000000000000000000000000000000000",
                    ]:
                        if url is not None:
                            percent = str('{:,}'.format(round(float(holder["percent"]) * 100))) + "%"
                            return hd.link(percent, url + holder["address"])
                        else:
                            return str('{:,}'.format(round(float(holder["percent"]) * 100))) + "%"
        if data.get("value_eth"):
            val = str('{:,}'.format(round(data['value_eth'])))
            return f"<b>{val} ETH</b>"
        return ""
    ...
    async def collect_info(self, data: dict, bot: Bot) -> dict:
        ...
        msg_data["name"] = (
            ...
            if data.get("token")
            else data["goplus"]["token_name"]
        )
        msg_data["symbol"] = (
            data["token"]["tokenSymbol"]
            if data.get("token")
            else data["goplus"]["token_symbol"]
        )
        ...
        msg_data["test"] = await self.get_message_analytic(
            data, msg_data.get("liquidity")
        )
    ...
    async def message_creater(self, data: dict, bot: Bot, address: str):
        ...
        if data.get("token"):
            TX = hd.link("TX", f"https://etherscan.io/tx/{data['token']['hash']}")
        else:
            TX = ""
        Verify = hd.link(
        ...
        message_template = (
            f"{msg_data['media']}"
            f"{msg_data['name']} ({msg_data['symbol']}) burned <b>"
            f"{msg_data['get_burned']}</b> liquidity!🔥 | {TX}\n"
