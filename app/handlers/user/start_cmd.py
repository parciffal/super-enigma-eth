...
async def message_template(bot_info):
    return (
        ...
        f"📊 This bot specializes in detecting and analyzing burn events on the Ethereum blockchain.\n"
        ...
        f"   Simply add bot to group, and the bot will check for recent burn events and provide you with a report! 📄\n"
        ...
        f"   We're constantly improving to provide you with the best burn detection experience on Ethereum! 🚀\n"
    )
    ...
    try:
        if message.text:
            ...
            if len(kls) == 42 and kls.startswith("0"):
                address = kls
                progress_msg: Message = await message.answer("🔥Burn Detector in progres on ETH 🔥🔍")
                (
                    msg,
                    keyboard
                ) = await token_analyzer.analyze({}, bot, config, address)
                await change_message(bot, progress_msg, msg, keyboard)
            elif kls.startswith("0") and (
                len(kls) < 42 or len(kls) > 42
            ):
